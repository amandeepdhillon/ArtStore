...
{  
   ...
   */
   public function __construct(array $data, $generateExc)
   {
      ...
      foreach ($data as $name => $value) {    
         $this->$name = $value; // this takes the fields of the table and assigns them as properties of the specific object on the fly
         // e.g. $this->ArtistID = 1
         // e.g. $this->LastName = "Picasso";
   ...
   */   
   ...
   public function jsonSerialize()
   {
      $className = get_class($this);
      $json = array();
      $searchItems = $className::getFieldNames();
      
      foreach ($searchItems as $field)
      {
         $lower = strtolower($field);
     
         if ( $this->doesFieldExist($field) ) {       
            if (!empty($this->$field)) {           
               $json[utf8_encode($field)] = utf8_encode($this->$field);
            }
         }
      }
